...
- **Топология** — Визуализация модулей / адаптеров / ботов и связей принадлежности их ресурсов и областей (требуется ErisPulse 2.8.0+)
...
- **Система владельцев** — Управление владельцами фреймворка с глобальными или платформенными правами, с отображением принадлежности провайдеров
- **Управление областями** — Трёхмерная контрольная плоскость (привязки модулей / допуск по личности / ограничения исходящих), глобальный переключатель default_allow, статистика, управление принадлежностью runtime-привязок и тестер определения (требуется ErisPulse 2.8.0+)
- **Управление командами** — Редактирование правил команд с отображением модуля-владельца, чёрных/белых списков ACL, переопределением параметров (master / hidden) и преобразованием команд
...
- **Единые UI-компоненты** — Пользовательские выпадающие списки и поисковые мультивыборы заменяют нативные элементы, сохраняя обратную совместимость
