...
(function () {
  ...
  I18N["ru"] = {
    ...
    settings_sync_framework_lang: "Синхронизировать",
    settings_sync_framework_label: "Синхронизировать язык с фреймворком",
    settings_sync_framework_desc: "Записывает текущий язык интерфейса панели в конфигурацию фреймворка; сообщения фреймворка и модулей будут использовать его",
    settings_sync_on_applied: "Применён глобальный вид",
    settings_sync_off_restored: "Синхронизация выключена, восстановлен локальный вид",
    settings_upload_confirm: "Текущий глобальный вид будет перезаписан и применён на всех устройствах с включённой синхронизацией. Продолжить?",
...
settings_tab_colorfont: "Цвет и шрифт",
    ...
    kb_focus_search: "Открыть глобальный поиск",
    ...
    settings_global_scope_desc: "Когда включено, это устройство автоматически применяет загруженный глобальный вид; когда выключено, восстанавливается локальный вид на момент включения",
    settings_upload_global_desc: "Загружает оформление этого устройства как глобальное; все устройства с включённой синхронизацией применят его",
    ...
    fw_update_done: "Обновление фреймворка завершено",
    update_hint_framework: "ErisPulse обновлён до v{v}. Рекомендуется перезапустить фреймворк, чтобы применить новые возможности и очистить кэш.",
    update_hint_dashboard: "Dashboard обновлён, его модуль перезагружен автоматически. При сбоях рекомендуется перезапустить фреймворк.",
    update_hint_restart: "Перезапустить",
    ...
    /* ── Гид по настройке после установки / онбординг (UX) ── */
    install_guide_title: "Установка завершена",
    install_guide_single: "«{name}» успешно установлен. Настроить сейчас?",
    install_guide_multi: "Следующие компоненты установлены и поддерживают настройку. Настроить сейчас?",
    install_guide_config_btn: "Настроить {name}",
    install_guide_later: "Позже",
    adapter_installed_toast: "Адаптер «{name}» установлен",
    goto_config: "Настроить",
    accounts_empty_title: "Аккаунт ещё не добавлен",
    accounts_empty_desc: "Адаптеру нужен хотя бы один аккаунт (бот) для подключения к платформе. После сохранения адаптер перезагрузится автоматически, настройки можно изменить в любой момент.",
    add_first_account: "Добавить аккаунт",
    view_docs: "Документация",
    /* ── Глобальный поиск ── */
    palette_placeholder: "Поиск страниц, компонентов, действий…",
    palette_empty: "Ничего не найдено",
    palette_group_pages: "Страницы",
    palette_group_components: "Компоненты",
    palette_group_actions: "Действия",
    palette_group_recent: "Недавние",
    palette_footer: "↑↓ выбор · Enter открыть · Esc закрыть",
    palette_open: "Глобальный поиск",
    sidebar_search: "Поиск",
    palette_act_theme: "Переключить тему",
    palette_act_lang: "Сменить язык",
    palette_act_restart: "Перезапустить фреймворк",
    palette_act_sidebar: "Свернуть / развернуть боковую панель",
    palette_act_privacy: "Переключить режим приватности",
    /* ── Защита от потери изменений ── */
    unsaved_confirm_title: "Есть несохранённые изменения",
    unsaved_confirm_text: "При уходе со страницы несохранённые изменения будут потеряны. Уйти?",
    /* ── Доп. тексты гида ── */
    install_guide_later_hint: "Настроить можно в любой момент в разделе «Конфигурация»",




    onboarding_title: "Быстрый старт",
    onboarding_subtitle: "Завершите первоначальную настройку для подключения бота",
    onboarding_step1: "Установите адаптер",
    onboarding_step2: "Добавьте аккаунт и завершите настройку",
    onboarding_step3: "Бот подключён",
    onboarding_progress: "Осталось шагов: {n}",
    onboarding_congrats: "Настройка завершена, бот в сети",
    ...
    font_upload: "Загрузить шрифт",
    font_delete: "Удалить шрифт",
    font_uploaded: "Шрифт загружен и применён",
    font_upload_failed: "Не удалось загрузить шрифт",
    settings_global_scope_desc: "Когда включено, все устройства автоматически синхронизируют все настройки ниже (внешний вид, поведение, язык, макет); когда выключено, каждое устройство использует локальные",
    settings_upload_global: "Отправить все настройки",
    settings_upload_global_desc: "Отправляет все настройки этого устройства как глобальные; все устройства с включённой синхронизацией применят их",
    settings_upload_confirm: "Глобальные настройки будут перезаписаны настройками этого устройства и применены на всех синхронизированных устройствах. Продолжить?",


    users_tokens_title: "Пользователи и токены",
    users_token_create: "Создать токен",
    users_token_name_ph: "Имя токена (напр. ПК)",
    users_tokens_list: "Созданные токены",
    users_token_caps_none: "Страницы не выданы",
    users_token_revoke: "Отозвать",
    users_tokens_empty: "Других токенов нет",
    users_token_name_required: "Укажите имя токена",
    users_token_created_hint: "Токен создан — скопируйте и сохраните сейчас (показывается один раз):",
    forbidden: "Нет прав для этого действия",


    settings_perms_tab: "Права",
    users_perm_advanced: "Права отдельных API",
    users_perm_advanced_desc: "Тонкая настройка по каждому API (переопределяет выбор на уровне страниц)",


    users_perm_no_view: "У этого токена нет доступа к этому представлению модуля",
