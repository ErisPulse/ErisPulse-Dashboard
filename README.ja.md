...
- **トポロジー** — モジュール / アダプター / Bot とそのリソース・スコープの帰属関係を可視化（ErisPulse 2.8.0+ 必須）
- **モジュール管理** — モジュールとアダプターの有効化、無効化、読み込み
- **プラグインストア** — リモートパッケージリポジトリの閲覧、オンラインでの依存パッケージのインストール
- **マスターシステム** — フレームワークのマスターを管理、グローバルまたはプラットフォーム別の権限設定をサポート、プロバイダーの帰属表示
- **スコープ管理** — 三次元コントロールプレーン（モジュールバインディング / ID アドミッション / 送信制限）、グローバル default_allow スイッチ、実行統計、ランタイムバインディングの帰属管理、判定テスター（ErisPulse 2.8.0+ 必須）
- **コマンド管理** — コマンドルール編集、帰属モジュール表示、ACL ユーザーブラック/ホワイトリスト、パラメータ上書き（master / hidden）、コマンド変換
- **設定編集** — 実行時にフレームワーク設定の表示と変更
- **ストレージ管理** — 永続化ストレージのキーバリューデータの表示、編集、削除
- **リモート再起動** — Web インターフェースからフレームワークを安全に再起動
- **クラスタ管理** — マルチノードクラスタの監視と管理
- **統一 UI コンポーネント** — カスタムドロップダウンと検索可能なマルチセレクトでネイティブコントロールを置き換え、後方互換を維持
